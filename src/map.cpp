		...
		*/
		...
		switch (liquid_type) {
			case LIQUID_SOURCE:
				...
				liquid_kind = nodemgr->getId(nodemgr->get(n0).liquid_alternative_flowing); // FIXME NDEF_GETID
				break;
			case LIQUID_FLOWING:
			...
			switch (nodemgr->get(nb.n.getContent()).liquid_type) {
				...
				case LIQUID_SOURCE:
					...
					if (liquid_kind == CONTENT_AIR)
						liquid_kind = nodemgr->getId(nodemgr->get(nb.n).liquid_alternative_flowing); // FIXME NDEF_GETID
					if (nodemgr->getId(nodemgr->get(nb.n).liquid_alternative_flowing) != liquid_kind) { // FIXME NDEF_GETID
						neutrals[num_neutrals++] = nb;
					} else {
		...
		if (num_sources >= 2 || liquid_type == LIQUID_SOURCE) {
			...
			new_node_content = nodemgr->getId(nodemgr->get(liquid_kind).liquid_alternative_source); // FIXME NDEF_GETID
