		...
		*/
		...
		for(s16 bz = -3; bz <= 3; bz++)
		{
			// X-
			{
				...
				v3f v(1, 0, 0);
				f32 dtime = 0;
				assert(axisAlignedCollision(s, m, v, dtime) == 0);
			...
			}
			{
				...
				v3f v(-1, 0, 0);
				f32 dtime = 0;
				assert(axisAlignedCollision(s, m, v, dtime) == -1);
			}
			{
				aabb3f s(bx, by, bz, bx+1, by+1, bz+1);
			...
			}
			{
				...
				v3f v(-0.5, 0.2, 0);
				f32 dtime;
				assert(axisAlignedCollision(s, m, v, dtime) == 1);  // Y, not X!
			...
			}
			{
				...
				v3f v(-1./3, -1./3, -1./3);
				f32 dtime;
				assert(axisAlignedCollision(s, m, v, dtime) == 2);
				assert(fabs(dtime - 0.9) < 0.001);
			}
			{
				aabb3f s(bx, by, bz, bx+2, by+2, bz+2);
